...
//    };
    ...
    protected int filterPixel(BufferedImage image, int x, int y) {
        int ip = image.getRGB(x, y);
        int[][] rgb = new int[2][3];
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                for (int k = 0; k <= 2; ++k) {
                    rgb[0][k] += GX[i][j] * component(image, x + i, y + j, k);
                    rgb[1][k] += GX[j][i] * component(image, x + i, y + j, k);
                }
            }
        }
...
//            rgb[0][k] = (int)Math.sqrt(rgb[0][k] * rgb[0][k] + rgb[1][k] + rgb[1][k]) & 0xFF;
    ...
    private int component(BufferedImage image, int x, int y, int offset) {
        x = x < 0 ? -x : (x >= image.getWidth()  ? 2 * image.getWidth() - x : x);
        y = y < 0 ? -y : (y >= image.getHeight() ? 2 * image.getHeight() - y: y);
